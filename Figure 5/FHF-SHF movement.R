...
#==============
resultTable <- read.csv(file = 'FHF-SHF movement.tsv', sep='\t')
...
#Plot displacement for all cells, complete tracks
...
test_1 <- filter(resultTable, str_detect(Region,"\\bJCF\\b") & End.T > 50 )
test_2  <- filter(resultTable, str_detect(Region,"aFHF") & End.T > 50  )
test_3  <- filter(resultTable, str_detect(Region,"aSHF") & End.T > 50  )
...
resultTable <- read.csv(file = 'FHF-SHF movement corrected.tsv', sep='\t')
